...
Chi tiết tại `docs/architecture-decisions.md`; mọi ADR là `PROPOSED` chờ con người xem xét.
...
Các điểm này chặn thiết kế chi tiết/kiểm thử tương ứng và không được triển khai bằng rule tự đặt. Chỉ người có thẩm quyền được cập nhật Architecture Gate trong `docs/human-gates.md`.
