...
Nếu timeout sau khi gửi mutation, copy phải nói “Chưa thể xác nhận thao tác đã hoàn tất”; reload dữ liệu nguồn trước khi cho retry. Cho tới khi durable idempotency contract ở `docs/backend-design.md` được duyệt/triển khai, không tự retry `POST/PATCH/DELETE` có thể gây lặp.
