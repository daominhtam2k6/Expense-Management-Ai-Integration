...
const progressOf = (goal: Goal) => goal.target_amount > 0 ? (goal.current_amount / goal.target_amount) * 100 : 0;
const remainingOf = (goal: Goal) => Math.max(0, goal.target_amount - goal.current_amount);
...
export function GoalsPage() {
  ...
  const removeGoal = async () => {
    if (!selectedGoal || !window.confirm(`Xóa mục tiêu “${selectedGoal.name}” cùng toàn bộ lịch sử đóng góp?`)) return;
    setSaving(true);
    setFormError("");
    try {
  ...
  return (
    <article className={`goal-card${attention ? " goal-card--attention" : ""}${goal.status === "completed" ? " goal-card--completed" : ""}`}>
      ...
      <div className="goal-card__main">
        ...
        <div className="goal-card__amount"><strong>{formatCurrency(goal.current_amount)}</strong><span>/ {formatCurrency(goal.target_amount)}</span></div>
        <div className="goal-card__progress"><span style={{ width: `${progress}%` }} /></div>
        <div className="goal-card__meta"><span>Còn thiếu {formatCurrency(remainingOf(goal))}</span><b>{Math.round(progress)}%</b></div>
      </div>
